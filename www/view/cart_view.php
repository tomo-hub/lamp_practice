  ...
  <div class="container">
    ...
    <?php if(count($carts) > 0){ ?>
      <table class="table table-bordered">
        ...
        <tbody>
          <!--データベースからとってきたカート情報を表示-->
          <?php foreach($carts as $cart){ ?>
          <tr>
            <td><img src="<?php print h(IMAGE_PATH . $cart['image']);?>" class="item_image"></td> <!--画像表示-->
            <td><?php print h($cart['name']); ?></td> <!--商品名表示-->
            <td><?php print h(number_format($cart['price'])); ?>円</td> <!--価格表示-->
            <td>
              <!--購入数変更-->
              <form method="post" action="cart_change_amount.php">
                <input type="number" name="amount" value="<?php print h($cart['amount']); ?>"> <!--購入数表示-->
                個
